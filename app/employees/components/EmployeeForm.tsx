        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <Input type="text" id="name" name="name" defaultValue={employee?.name} required placeholder="Misal: Budi Santoso"/>
            </div>

            <div className="space-y-2">
              <label htmlFor="email" className="text-sm font-medium text-slate-700">Alamat Email <span className="text-red-500">*</span></label>
              <Input type="email" id="email" name="email" defaultValue={employee?.email} required placeholder="budi.santoso@perusahaan.com"/>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-6 p-2">
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input type="radio" name="gender" value="male" defaultChecked={employee?.gender === "male" || !employee} className="w-4 h-4 text-teal-600 border-slate-300 focus:ring-teal-500 transition-colors" />
                  <span className="text-sm text-slate-700 group-hover:text-slate-900 font-medium">Laki-laki</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input type="radio" name="gender" value="female" defaultChecked={employee?.gender === "female"} className="w-4 h-4 text-teal-600 border-slate-300 focus:ring-teal-500 transition-colors" />
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <Select id="departmentId" value={selectedDeptId} onChange={(e) => setSelectedDeptId(Number(e.target.value))} required>
            ...
            <div className="space-y-2">
              ...
              <Select id="positionId" name="positionId" defaultValue={employee?.positionId || ""} required disabled={!selectedDeptId}>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3">
            {skills.map(skill => {
              ...
              return (
                <label key={skill.id} className="flex items-center gap-2.5 cursor-pointer bg-white border border-slate-200 px-4 py-2.5 rounded-xl hover:bg-slate-50 hover:border-slate-300 transition-all has-[:checked]:bg-teal-50 has-[:checked]:border-teal-300 shadow-sm">
                  <input type="checkbox" name="skills" value={skill.id} defaultChecked={isChecked} className="w-4 h-4 text-teal-600 rounded border-slate-300 focus:ring-teal-500" />
